...
import { doc, getDoc, updateDoc, collection, query, where, orderBy, onSnapshot, addDoc, serverTimestamp, deleteDoc } from 'firebase/firestore';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import { useAuth } from '../hooks/useAuth';
import { useUpload } from '../hooks/useUpload';
import { UserProfile, Post } from '../types';
import PostCard from '../components/PostCard';
import { Edit3, Camera, MapPin, Book, Calendar, Mail, CheckCircle, GraduationCap, PenTool, UserPlus, UserCheck, UserX, Clock, Phone, Eye, EyeOff, Bell, Droplets, Dumbbell, Plus, Minus, ShoppingBag } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-slate-900 rounded-3xl shadow-xl border border-slate-800 overflow-hidden">
        <div className="h-48 bg-gradient-to-r from-purple-600 to-indigo-900 relative">
          <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>
          {isOwner && (
            <button 
              onClick={() => setShowSouqi(true)}
              className="absolute top-6 right-6 bg-amber-500 hover:bg-amber-400 text-white px-4 py-2 rounded-xl font-black shadow-lg shadow-amber-500/20 transition-all flex items-center gap-2 active:scale-95 z-10"
            >
              <ShoppingBag className="w-5 h-5" /> Souqi - سوقي
            </button>
          )}
        </div>
            ...
            {isEditing ? (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    className="w-full px-4 py-3 bg-slate-900 border border-slate-800 text-white rounded-2xl outline-none focus:ring-2 focus:ring-purple-500 transition-all font-bold min-h-[100px]"
                    value={editData.bio}
              ...
              >
                <div className="md:col-span-2 space-y-4">
                  ...
                  <p className="text-slate-400 font-medium leading-relaxed">
                    {profile.bio || "No bio provided yet. This teacher is busy educating the next generation!"}
                  </p>
                </div>
